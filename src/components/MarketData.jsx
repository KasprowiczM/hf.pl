          ...
          <section className="interactive-card reveal reveal-right rounded-[2rem] border border-border bg-surface-offset px-6 py-7 text-white sm:px-8">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {benchmarks.map((benchmark) => (
                <div key={benchmark.key} className="rounded-[1.4rem] border border-white/10 bg-white/5 px-5 py-5">
                  <p className="text-xs uppercase tracking-[0.18em] text-white/45">{t(benchmark.key)}</p>
                  <p className="stat-value mt-3 text-[2rem] leading-none text-white">{t(benchmark.valueKey)}</p>
                </div>
              ))}
            </div>
          </section>
